...
public class Yaff {
    ...
    private final HashMap<Integer, IYaffSerializer<?>> serializers; // Class Hash -> Serializer
    private final HashMap<Integer, Integer> typeId; // Network Identifier -> Class Hash
    ...
    public void registerContentType(Type type, IYaffSerializer<?> serializer) {
        if (serializer == null) throw new RuntimeException("Serializer cannot be null");

        serializers.put(type.getTypeName().hashCode(), serializer);
        typeId.put(serializer.getUniqueIdentifier(), type.getTypeName().hashCode());
    }

    public byte[] serialize(Object object) {
        var serializer = (IYaffSerializer<Object>) serializers.get(object.getClass().getTypeName().hashCode());
    ...
    public Object deserialize(byte[] data) throws YaffException {
        ...
        var serializer = (IYaffSerializer<Object>) serializers.get(typeId.get(networkId));
    ...
    public Object deserialize(InputStream stream) throws YaffException, IOException {
        ...
        if (stream.read(data) != dataLength) throw new YaffException("Failed to read data");
